...
object MostPopularSuperhero {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("../extras/Marvel-names.txt")
    val namesRdd = names.flatMap(parseNames)

    // Load up the superhero co-apperarance data
    val lines = sc.textFile("../extras/Marvel-graph.txt")
    ...
    val flipped = totalFriendsByCharacter.map( x => (x._2, x._1) ).cache()
...
//    val sortedByPopularity = flipped.sortByKey(ascending = false).collect()
//    val top10 = sortedByPopularity.take(10).map(x => (x._1, namesRdd.lookup(x._2).head))
//    val last10 = sortedByPopularity.takeRight(20).map(x => (x._1, namesRdd.lookup(x._2).head))

    val sortedByPopularity = flipped.sortByKey()
    val top10 = sortedByPopularity.top(10).map(x => (x._1, namesRdd.lookup(x._2).head))
    val last10 = sortedByPopularity.take(20).map(x => (x._1, namesRdd.lookup(x._2).head))
