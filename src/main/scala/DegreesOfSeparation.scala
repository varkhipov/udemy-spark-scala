...
//    for ( connection <- 1 to (fields.length - 1)) {
  ...
  def bfsReduce(data1:BFSData, data2:BFSData): BFSData = {
    ...
    // Reduced node values
    var minimumDistance:Int = 9999 // used to emulate infinity
    var darkestColor:String = "WHITE"
    var allConnections:ArrayBuffer[Int] = ArrayBuffer()

    // See if one is the original node with its connections.
    // If so preserve them.
    if (connections1.length > 0) {
      allConnections ++= connections1
    }
    if (connections2.length > 0) {
      allConnections ++= connections2
    }
    ...
    (allConnections.toArray, minimumDistance, darkestColor)
  }
  ...
  def main(args: Array[String]) {
    ...
    // TODO: Show acquaintance sequence
  }

  /** Create "iteration 0" of our RDD of BFSNodes */
  def createStartingRdd(sc:SparkContext): RDD[BFSNode] = {
    val inputFile = sc.textFile("../extras/Marvel-graph.txt")
